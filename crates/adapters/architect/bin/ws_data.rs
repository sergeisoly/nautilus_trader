...
//!
//! For 2FA (if enabled on your account), provide ONE of:
//! - `ARCHITECT_TOTP_SECRET`: Base32 secret for auto-generating codes (recommended)
//! - `ARCHITECT_TOTP`: Pre-generated 6-digit code
//!
//! Usage:
//! ```bash
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let totp = std::env::var("ARCHITECT_TOTP").ok();
    ...
    // Check for TOTP secret (to auto-generate codes) or pre-provided code
    let totp_secret = std::env::var("ARCHITECT_TOTP_SECRET").ok();
    let totp_code: Option<String> = if let Some(ref secret) = totp_secret {
        ...
        Err(e) => {
            ...
            if matches!(e, ArchitectHttpError::UnexpectedStatus { status: 400, .. }) {
                let code = match totp_code {
                    Some(code) => code,
                    None => {
                        tracing::error!("2FA required but no TOTP code available");
                        tracing::error!(
                            "Set ARCHITECT_TOTP_SECRET (base32 secret) or ARCHITECT_TOTP (6-digit code)"
                        );
                        return Err("2FA required but no TOTP provided".into());
                    }
                };
