...
pub struct GetCandlesParams {
    ...
    pub candle_width: ArchitectCandleWidth,
}
...
impl GetCandleParams {
    ...
    #[must_use]
    pub fn new(symbol: impl Into<String>, candle_width: ArchitectCandleWidth) -> Self {
        Self {
            symbol: symbol.into(),
            candle_width,
        }
...
mod tests {
    ...
    fn test_get_candle_params_serialization() {
        let params = GetCandleParams::new("GBPUSD-PERP", ArchitectCandleWidth::Hours1);
        let qs = serde_urlencoded::to_string(&params).unwrap();
        assert!(qs.contains("symbol=GBPUSD-PERP"));
