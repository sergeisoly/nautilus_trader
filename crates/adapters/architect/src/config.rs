...
//! Configuration structures for the Architect adapter.

use nautilus_model::identifiers::AccountId;

/// Configuration for the Architect live data client.
#[derive(Clone, Debug)]
pub struct ArchitectDataClientConfig {
...
impl Default for ArchitectDataClientConfig {
    fn default() -> Self {
        Self {
            api_key: None,
            api_secret: None,
...
impl ArchitectDataClientConfig {
    ...
    pub fn http_base_url(&self) -> String {
        self.base_url_http.clone().unwrap_or_else(|| {
            if self.is_sandbox {
                "https://sandbox.x.architect.co/api".to_string()
            } else {
                "https://x.architect.co/api".to_string()
            }
        })
    }
    ...
    pub fn ws_public_url(&self) -> String {
        self.base_url_ws_public.clone().unwrap_or_else(|| {
            if self.is_sandbox {
                "wss://sandbox.x.architect.co/ws/public".to_string()
            } else {
                "wss://x.architect.co/ws/public".to_string()
            }
        })
    }
    ...
    pub fn ws_private_url(&self) -> String {
        self.base_url_ws_private.clone().unwrap_or_else(|| {
            if self.is_sandbox {
                "wss://sandbox.x.architect.co/ws/private".to_string()
            } else {
                "wss://x.architect.co/ws/private".to_string()
            }
        })
    }
...
/// Configuration for the Architect live execution client.
#[derive(Clone, Debug)]
pub struct ArchitectExecClientConfig {
...
impl Default for ArchitectExecClientConfig {
    fn default() -> Self {
        Self {
            api_key: None,
            api_secret: None,
...
impl ArchitectExecClientConfig {
    /// Creates a configuration with default values.
    #[must_use]
    pub fn new() -> Self {
        Self::default()
