# -------------------------------------------------------------------------------------------------
#  Copyright (C) 2015-2025 Nautech Systems Pty Ltd. All rights reserved.
#  https://nautechsystems.io
#
#  Licensed under the GNU Lesser General Public License Version 3.0 (the "License");
#  You may not use this file except in compliance with the License.
#  You may obtain a copy of the License at https://www.gnu.org/licenses/lgpl-3.0.en.html
#
#  Unless required by applicable law or agreed to in writing, software
#  distributed under the License is distributed on an "AS IS" BASIS,
#  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#  See the License for the specific language governing permissions and
#  limitations under the License.
# -------------------------------------------------------------------------------------------------

from __future__ import annotations

from nautilus_trader.adapters.aster.constants import (
    ASTER_BASE_URL_HTTP,
    ASTER_BASE_URL_WS,
    ASTER_VENUE,
)
from nautilus_trader.adapters.binance.config import (
    BinanceDataClientConfig,
    BinanceExecClientConfig,
    BinanceInstrumentProviderConfig,
)
from nautilus_trader.model.identifiers import Venue


class AsterInstrumentProviderConfig(BinanceInstrumentProviderConfig, frozen=True):
    """
    Configuration for ``AsterInstrumentProvider`` instances.
    """

    venue: Venue = ASTER_VENUE
    base_url_http: str = ASTER_BASE_URL_HTTP
    log_warnings: bool = True


class AsterDataClientConfig(BinanceDataClientConfig, frozen=True):
    """
    Configuration for ``AsterMarketDataClient`` instances.
    """

    venue: Venue = ASTER_VENUE
    base_url_http: str | None = ASTER_BASE_URL_HTTP
    base_url_ws: str | None = ASTER_BASE_URL_WS
    use_agg_trade_ticks: bool = True


class AsterExecClientConfig(BinanceExecClientConfig, frozen=True):
    """
    Configuration for ``AsterExecutionClient`` instances.
    """

    venue: Venue = ASTER_VENUE
    base_url_http: str | None = ASTER_BASE_URL_HTTP
    base_url_ws: str | None = ASTER_BASE_URL_WS
